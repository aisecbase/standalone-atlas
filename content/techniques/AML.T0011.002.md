...
description: Жертва может вызвать отравленный инструмент при взаимодействии с ИИ-агентом. В результате вызова агент может подвергнуться воздействию отравленных определений или ответов, может быть выполнена вредоносная логика,...
generated: true
...
Жертва может вызвать отравленный инструмент при взаимодействии с ИИ-агентом. В результате вызова агент может подвергнуться воздействию отравленных определений или ответов, может быть выполнена вредоносная логика, заложенная в реализацию инструмента, либо агент может вызвать дополнительные инструменты. Отравленные инструменты могут попадать в систему через ПО для ИИ, установленные пакеты и навыки либо при подключении к удалённо размещённым сервисам.
