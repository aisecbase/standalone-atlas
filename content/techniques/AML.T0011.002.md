...
description: Жертва может вызвать отравленный инструмент при взаимодействии с ИИ-агентом. Вызов может привести к тому, что агент получит отравленные определения или ответы, выполнится вредоносная логика, заложенная в реализацию...
generated: true
...
Жертва может вызвать отравленный инструмент при взаимодействии с ИИ-агентом. Вызов может привести к тому, что агент получит отравленные определения или ответы, выполнится вредоносная логика, заложенная в реализацию инструмента, либо агент вызовет дополнительные инструменты. Отравленные инструменты могут попадать в систему через ПО для ИИ, установленные пакеты и навыки либо при подключении к удалённо размещённым сервисам.
